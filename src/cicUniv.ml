...
(*                                                                           *)
(*****************************************************************************)
(* $Id: cicUniv.ml 9491 2009-01-15 15:31:25Z tassi $ *)
(*****************************************************************************)
(** open                                                                    **)
(*****************************************************************************)
open Printf
...
module UniverseType = struct type t = universe
                              let compare = compare
                                 end
...
type entry =
  { eq_closure : SOF.t; ge_closure : SOF.t; gt_closure : SOF.t;
    in_gegt_of : SOF.t; one_s_eq : SOF.t; one_s_ge : SOF.t; one_s_gt : SOF.t
  }
...
let are_set_eq s1 s2 = SOF.equal s1 s2
...
let string_of_node n =
  "{" ^
    ("eq_c: " ^
       ((string_of_universe_set n.eq_closure) ^
          ("; " ^
             ("ge_c: " ^
                ((string_of_universe_set n.ge_closure) ^
                   ("; " ^
                      ("gt_c: " ^
                         ((string_of_universe_set n.gt_closure) ^
                            ("; " ^
                               ("i_gegt: " ^
                                  ((string_of_universe_set n.in_gegt_of) ^
                                     "}\n")))))))))))
  ...
  in
    (MAL.iter
       (fun k v ->
          rc :=
            !rc ^
              (sprintf "%s --> %s" (string_of_universe k) (string_of_node v)))
       m;
     !rc)
  
let string_of_bag b = string_of_mal b
  
(*****************************************************************************)
(** Helpers                                                                 **)
(*****************************************************************************)
(* find the repr *)
let repr u m = try MAL.find u m with | Not_found -> empty_entry
...
and closure_eq_fast ru m =
  let eq_c =
    ...
    let one_step_eq = ru.one_s_eq in SOF.union one_step_eq _Uj
  in eq_c
and closure_ge_fast ru m =
  let ge_c =
    ...
    let _Ux = j in SOF.union _Uj _Ux
  in ge_c
and closure_gt_fast ru m =
  let gt_c =
    ...
    let one_step_gt = ru.one_s_gt
    in SOF.union (SOF.union (SOF.union _Ul one_step_gt) _Uk) _Uj
  in gt_c
...
and adjust_fast_aux adjusted u m =
  ...
  then (m, adjusted)
    ...
    (let adjusted = SOF.add u adjusted in
     ...
     in
       if (not changed_gegt) && (not changed_eq)
       then (m, adjusted)
       else
         (let ru' =
            {
          ...
          let (m, adjusted) =
            SOF.fold
              (fun x (m, adjusted) ->
                 ((MAL.add x ru' m), (SOF.add x adjusted)))
              (SOF.diff ru'.eq_closure adjusted) (m, adjusted) in
          let (m, adjusted) =
            SOF.fold (fun x (m, adjusted) -> adjust_fast_aux adjusted x m)
              (SOF.diff ru'.in_gegt_of adjusted) (m, adjusted)
          in (m, adjusted)))
and
  (*
...
*)
  adjust_fast x y = fst (adjust_fast_aux SOF.empty x y)
and add_gt_arc_fast u v m =
  let ru = repr u m
  in
    if SOF.mem v ru.gt_closure
    then m
    else
      (let ru' = { (ru) with one_s_gt = SOF.add v ru.one_s_gt; } in
       ...
       let m'' = MAL.add v rv' m' in adjust_fast u m'')
and add_ge_arc_fast u v m =
  let ru = repr u m
  in
    if SOF.mem v ru.ge_closure
    then m
    else
  ...
  in
    ...
    then m
      ...
      (let rv = repr v m in
       ...
       let m'' = MAL.add v rv' m' in adjust_fast v (*(adjust_fast u*) m'')
                                                   ...
                                                   ("   of\n" ^
                                                      ("\t" ^
                                                         ((string_of_universe
                                                            ...
                                                            ("\n\n" ^
                                                               "  ===== Universe Inconsistency detected =====\n")))))))))))))))))))
  in (*   prerr_endline (Lazy.force s); *) raise (UniverseInconsistency s)
  
let fill_empty_nodes_with_uri (g, already_contained, o) l uri =
  let fill_empty_universe u =
    match u with | (i, None) -> (i, (Some uri)) | ((i, Some _) as u) -> u in
  let fill_empty_set s =
    SOF.fold (fun e s -> SOF.add (fill_empty_universe e) s) s SOF.empty in
  ...
  let m' =
    MAL.fold
      (fun k v m -> MAL.add (fill_empty_universe k) (fill_empty_entry v) m) m
      MAL.empty in
  let l' = List.map fill_empty_universe l in ((m', already_contained, o), l')
...
let fresh ?uri ?id () =
  let i =
    match (uri, id) with
    | (None, None) ->
        (current_index_anon := !current_index_anon + 1; !current_index_anon)
...
let name_universe u uri =
  match u with | (i, None) -> (i, (Some uri)) | u -> u
  
let print_ugraph (g, _, o) =
  if o
  then prerr_endline "oblivion universe"
  else prerr_endline (string_of_bag g)
  
let add_eq u v b = (* should we check to no add twice the same?? *)
  let m = b in
  let ru = repr u m
  in
    if SOF.mem v ru.gt_closure
    then error ("EQ", u, v) v "GT" u ru.gt_closure
    else
      (let rv = repr v m
       in
         if SOF.mem u rv.gt_closure
         then error ("EQ", u, v) u "GT" v rv.gt_closure
         else add_eq_arc_fast u v b)
  
let add_ge u v b = (* should we check to no add twice the same?? *)
  let m = b in
  let rv = repr v m
  in
    if SOF.mem u rv.gt_closure
    then error ("GE", u, v) u "GT" v rv.gt_closure
    else add_ge_arc_fast u v b
  
let add_gt u v b = (* should we check to no add twice the same?? *)
  (* 
  ...
  *)
  ...
  in
    if u = v
    then error ("GT", u, v) u "==" v SOF.empty
    else
      (*if SOF.mem u rv.gt_closure then
    error ("GT",u,v) u "GT" v rv.gt_closure
  else
    begin*)
      if SOF.mem u rv.ge_closure
      then error ("GT", u, v) u "GE" v rv.ge_closure
      else
        (*        begin
...
let add_eq u v (b, already_contained, oblivion) =
  if oblivion
  then (b, already_contained, oblivion)
  else (let rc = add_eq u v b in (rc, already_contained, false))
  
let add_ge u v (b, already_contained, oblivion) =
  if oblivion
  then (b, already_contained, oblivion)
  else (let rc = add_ge u v b in (rc, already_contained, false))
  
let add_gt u v (b, already_contained, oblivion) =
  if oblivion
  then (b, already_contained, oblivion)
  else (let rc = add_gt u v b in (rc, already_contained, false))
...
let add_gt u v b = profiler_gt.HExtlib.profile (fun _ -> add_gt u v b) ()
  
let add_ge u v b = profiler_ge.HExtlib.profile (fun _ -> add_ge u v b) ()
  
let add_eq u v b = profiler_eq.HExtlib.profile (fun _ -> add_eq u v b) ()
...
let do_rank (b, _, _) =
  ...
  let rec aux cache l =
    ...
    | x :: tl when List.mem_assoc x cache ->
        ...
        let (rest, cache) = aux cache tl in ((max rest height), cache)
    | x :: tl ->
        let sons = SOF.elements (repr x b).gt_closure in
        let (height, cache) = aux cache sons in
        let height = height + 1 in
        let cache = (x, height) :: cache in
        let (rest, cache) = aux cache tl in ((max height rest), cache) in
  let (_, cache) = aux [] keys
  in
    (rank := List.fold_left (fun m (k, v) -> MAL.add k v m) MAL.empty cache;
     ...
     in
       (MAL.iter
          (fun k v ->
             (if not (List.mem v !res) then res := v :: !res else ();
              resk := k :: !resk))
          !rank;
        ((!res), (!resk))))
  
let get_rank u = try MAL.find u !rank with | Not_found -> 0
...
let xml_of_universe name u =
  ...
  | (_, None) -> raise (Failure "we can serialize only universes with uri")
  
let xml_of_set s =
  let l = List.map (xml_of_universe "node") (SOF.elements s)
  in List.fold_left (fun s x -> Stream.iapp s x) Stream.sempty l
  
let xml_of_entry_content e =
  let stream_of_field f name =
    let eq_c = xml_of_set f
    in
      if ( != ) eq_c Stream.sempty
      then Xml.xml_nempty name [] eq_c
      else Stream.sempty
  in
    Stream.lapp (fun _ -> stream_of_field e.eq_closure "eq_closure")
      (Stream.lapp (fun _ -> stream_of_field e.gt_closure "gt_closure")
         (Stream.lapp (fun _ -> stream_of_field e.ge_closure "ge_closure")
            (Stream.lapp (fun _ -> stream_of_field e.in_gegt_of "in_gegt_of")
               (Stream.lapp (fun _ -> stream_of_field e.one_s_eq "one_s_eq")
                  (Stream.lapp
                     (fun _ -> stream_of_field e.one_s_gt "one_s_gt")
                     (Stream.slazy
                        (fun _ -> stream_of_field e.one_s_ge "one_s_ge")))))))
...
let xml_of_entry u e =
  ...
  let u'' =
    ...
    | Some x -> x
    | None ->
        raise (Failure "we can serialize only universes (entry) with uri") in
  let ent =
    Xml.xml_nempty "entry"
      [ (None, "id", (string_of_int i));
        (None, "uri", (UriManager.string_of_uri u'')) ] in
  let content = xml_of_entry_content e in ent content
              ...
              (Stream.lapp
                 (fun _ ->
                    MAL.fold
                      (fun k v s ->
                         Stream.iapp s
                           (Stream.slazy (fun _ -> xml_of_entry k v)))
                      m Stream.sempty)
                 (Stream.slazy
                    (fun _ ->
                       List.fold_left
                         (fun s u ->
                            Stream.iapp s
                              (Stream.slazy
                                 (fun _ -> xml_of_universe "owned_node" u)))
                         Stream.sempty l)))))
  in Xml.pp ~gzip: true tokens (Some filename)
...
let rec clean_ugraph m already_contained f =
  ...
  let e_l =
    MAL.fold
      (fun k v l ->
         if (v = empty_entry) && (not (f k)) then SOF.add k l else l)
      m'' SOF.empty
  in
    if not (SOF.is_empty e_l)
    then
      clean_ugraph m'' already_contained
        (fun u -> (f u) && (not (SOF.mem u e_l)))
    else
      ((MAL.fold (fun k v x -> if v <> empty_entry then MAL.add k v x else x)
          m'' MAL.empty),
       already_contained)
...
let clean_ugraph (m, a, o) l =
  (assert (not o);
   ...
   let (m, a) = clean_ugraph m a (fun u -> SOF.mem u l) in (m, a, o))
  
let assigner_of =
  function
  | "ge_closure" ->
      (fun e u -> { (e) with ge_closure = SOF.add u e.ge_closure; })
  | "gt_closure" ->
      (fun e u -> { (e) with gt_closure = SOF.add u e.gt_closure; })
  | "eq_closure" ->
      (fun e u -> { (e) with eq_closure = SOF.add u e.eq_closure; })
  | "in_gegt_of" ->
      (fun e u -> { (e) with in_gegt_of = SOF.add u e.in_gegt_of; })
...
let cb_factory m l = let module XPP = XmlPushParser
  in
    let current_node = ref (0, None) in
    let current_entry = ref empty_entry in
    let current_assign = ref (assigner_of "in_gegt_of")
    in
      {
        (XPP.
        default_callbacks)
          ...
          Some
            (fun name attlist ->
               ...
               | "entry" ->
                   ...
                   let uri = List.assoc "uri" attlist
                   in
                     current_node :=
                       ((int_of_string id),
                        (Some (UriManager.uri_of_string uri)))
               | "node" ->
                   let id = int_of_string (List.assoc "id" attlist) in
                   let uri = List.assoc "uri" attlist
                   in
                     current_entry :=
                       !current_assign !current_entry
                         (id, (Some (UriManager.uri_of_string uri)))
               | "owned_node" ->
                   let id = int_of_string (List.assoc "id" attlist) in
                   let uri = List.assoc "uri" attlist
                   in l := (id, (Some (UriManager.uri_of_string uri))) :: !l
               | s -> current_assign := assigner_of s);
      }
  
let ugraph_and_univlist_of_xml filename = let module XPP = XmlPushParser
  in
    ...
    let xml_source = `Gzip_file filename
    in
...
let assert_univs_have_uri (graph, _, _) univlist =
  let assert_set s = SOF.iter (fun u -> assert_univ u) s in
  let assert_entry e =
