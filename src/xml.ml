...
type token =
  | Str of string
  | Empty of string option * string
             * ((string option) * string * string) list
  | NEmpty of string option * string
              * ((string option) * string * string) list * token Stream.t

(* currified versions of the constructors make the code more readable *)
let xml_empty ?prefix name attrs = Stream.ising (Empty (prefix, name, attrs))
  
let xml_nempty ?prefix name attrs content =
  Stream.ising (NEmpty (prefix, name, attrs, content))
  
let xml_cdata str = Stream.ising (Str str)
  
(** low level for other PPs: pretty print each token of strm applying 'f' to a
canonical string representation of each token *)
let pp_gen f strm =
  let pprefix = function | None -> "" | Some p -> p ^ ":" in
  let rec pp_r m (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some (Str a) ->
        (Stream.junk __strm;
         let s = __strm in (print_spaces m; f (a ^ "\n"); pp_r m s))
    | Some (Empty (p, n, l)) ->
        (Stream.junk __strm;
         let s = __strm
         in
        ...
        (Stream.junk __strm;
         ...
         in
           (print_spaces m;
            ...
            pp_r m s))
    | _ -> ()
  and print_spaces m = for i = 1 to m do f "  " done
  in pp_r 0 strm
  
(** pretty printer on output channels *)
let pp_to_outchan strm oc =
  (pp_gen (fun s -> output_string oc s) strm; flush oc)
  
let pp_to_gzipchan strm oc =
  pp_gen (fun s -> Gzip.output oc (Bytes.of_string s) 0 (String.length s))
    strm
  
(** pretty printer to string *)
let pp_to_string strm =
  let buf = Buffer.create 10240
  in (pp_gen (Buffer.add_string buf) strm; Buffer.contents buf)
...
let pp ?(gzip = false) strm fn =
  ...
  then
    (match fn with
     | Some filename ->
         let outchan = Gzip.open_out filename
         in
           ((try pp_to_gzipchan strm outchan
             with | e -> (Gzip.close_out outchan; raise e));
    ...
    (match fn with
     | Some filename ->
         ...
         in
           ((try pp_to_outchan strm outchan
...
let pp =
  let profiler = HExtlib.profile "Xml.pp"
  in fun ?gzip strm fn -> profiler.HExtlib.profile (pp ?gzip strm) fn
  
let add_xml_declaration stream =
  let box_prefix = "b"
  in
    Stream.lapp (fun _ -> xml_cdata "<?xml version=\"1.0\" ?>\n")
      (Stream.lapp (fun _ -> xml_cdata "\n")
         (Stream.slazy
            (fun _ ->
...
let strip_xml_headings s =
  ...
  in try aux 4 0 with | Not_found -> s
